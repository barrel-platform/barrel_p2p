...
-type stream_ref() :: {mycelium_dist_stream, node(), non_neg_integer()}.
        ...
        ok ->
            ...
            case start_quic_server(Name, Port, Config, ExtraOpts) of
                {ok, ServerName, ActualPort} ->
                    ...
                    Listener = #mycelium_dist_listener{
...
accept(#mycelium_dist_listener{server_name = ServerName} = Listener) ->
    ...
    persistent_term:put({mycelium_dist_acceptor, ServerName}, AcceptorPid),
    AcceptorPid.
...
accept_connection(_AcceptPid, DistCtrl, MyNode, Allowed, SetupTime) ->
    ...
    spawn_opt(
        fun() ->
            ...
            ok = mycelium_dist_controller:set_supervisor(DistCtrl, Kernel),
...
ensure_quic_minimal() ->
    ...
    case code:ensure_loaded(crypto) of
        {module, crypto} ->
            ...
            put(mycelium_dist_early_boot, true),
            ok;
        {error, Reason} ->
...
%% Command-line arguments take precedence (using -mycelium_dist key value format).
load_config() ->
    %% First check init arguments (command line), then fall back to app env
    DistOpts = application:get_env(mycelium, dist, []),

    #mycelium_dist_config{
        ...
        discovery_module = get_opt(discovery_module, DistOpts, mycelium_quic_discovery),
        ...
        congestion_threshold = get_opt(
            congestion_threshold, DistOpts, ?MYCELIUM_DEFAULT_QUEUE_CONGESTION_THRESHOLD
        ),
        max_pull_per_notification = get_opt(
            max_pull_per_notification, DistOpts, ?MYCELIUM_DEFAULT_MAX_PULL_PER_NOTIFICATION
        ),
        backpressure_retry_ms = get_opt(
            backpressure_retry_ms, DistOpts, ?MYCELIUM_DEFAULT_BACKPRESSURE_RETRY_MS
        ),
        %% Pacing
...
%% @private
%% Get value from init argument -mycelium_dist_Key Value
get_init_arg(Key, Default) ->
    ArgName = list_to_atom("mycelium_dist_" ++ atom_to_list(Key)),
    case init:get_argument(ArgName) of
        {ok, [[Value]]} ->
            Value;
        _ ->
            %% Also try plain -mycelium_dist with key value pairs
            case init:get_argument(mycelium_dist) of
                {ok, Args} ->
...
start_quic_server(Name, Port, Config, _ExtraOpts) ->
    ...
    case load_credentials(Config) of
        {ok, Cert, Key, _CACert} ->
            CongestionThreshold = Config#mycelium_dist_config.congestion_threshold,
            Opts = #{
                cert => Cert,
                key => Key,
                alpn => [?MYCELIUM_DIST_ALPN],
                idle_timeout => ?MYCELIUM_DIST_IDLE_TIMEOUT,
                %% QUIC-level keep-alive via PING frames (bypasses flow control)
                keep_alive_interval => ?MYCELIUM_DIST_KEEP_ALIVE_INTERVAL,
                ...
                pacing_enabled => Config#mycelium_dist_config.pacing_enabled,
...
register_early_boot_listener(Name, Pid, Port) ->
    ...
    Key = {mycelium_dist_early_listener, Name},
...
load_credentials(#mycelium_dist_config{cert = Cert, key = Key, cacert = CACert}) when
    Cert =/= undefined, Key =/= undefined
->
    {ok, Cert, Key, CACert};
load_credentials(#mycelium_dist_config{
    cert_file = CertFile,
    key_file = KeyFile,
...
load_credentials(_) ->
    %% No explicit cert configured - fall back to a self-signed cert
    %% generated under the data dir via mycelium_quic_cert. Useful for
    %% dev/test setups where TLS exists only for transport encryption
    %% (peer identity is verified separately by mycelium_dist_auth).
    case mycelium_quic_cert:ensure_cert() of
        ok ->
            {CertFile, KeyFile} = mycelium_quic_cert:get_cert_paths(),
            load_credentials(#mycelium_dist_config{
                cert_file = CertFile,
                key_file = KeyFile
            });
...
acceptor_loop(Kernel, #mycelium_dist_listener{} = Listener) ->
    receive
...
discover_node(Node, Host) ->
    Config = load_config(),
    DiscoveryModule = Config#mycelium_dist_config.discovery_module,

    %% First check static configuration
    case lists:keyfind(Node, 1, Config#mycelium_dist_config.nodes) of
...
resolve_host(Host) ->
    case inet:getaddr(Host, inet) of
        {ok, IP} ->
            {ok, IP, ?MYCELIUM_DIST_DEFAULT_PORT};
        {error, _} ->
...
set_connect_options(Node, Opts) when is_atom(Node), is_map(Opts) ->
    ...
    true = ets:insert(mycelium_dist_connect_opts, {Node, Opts}),
    ok.
...
get_connect_options(Node) when is_atom(Node) ->
    case ets:info(mycelium_dist_connect_opts) of
        undefined ->
            #{};
        _ ->
            case ets:lookup(mycelium_dist_connect_opts, Node) of
                [{Node, Opts}] -> Opts;
                [] -> #{}
            end
    end.
...
take_connect_options(Node) ->
    case ets:info(mycelium_dist_connect_opts) of
        undefined ->
            #{};
        _ ->
            case ets:take(mycelium_dist_connect_opts, Node) of
                [{Node, Opts}] -> Opts;
                [] -> #{}
            end
    end.
...
ensure_connect_opts_table() ->
    ensure_named_set(mycelium_dist_connect_opts).
...
create_hs_data(DistCtrl, MyNode, Timer, Allowed, Kernel) ->
    ...
    #hs_data{
        ...
        this_flags = 0,
        other_flags = 0,
        f_send = fun(Ctrl, Data) -> mycelium_dist_controller:send(Ctrl, Data) end,
        f_recv = fun(Ctrl, Len, Timeout) ->
            %% Receive data and try to extract node name if this is the name message
            Result = mycelium_dist_controller:recv(Ctrl, Len, Timeout),
            case Result of
                {ok, Data} ->
        ...
        f_address = fun(Ctrl, Node) ->
            mycelium_dist_controller:get_address(Ctrl, Node)
        end,
        mf_tick = fun(Ctrl) -> mycelium_dist_controller:tick(Ctrl) end,
        mf_getstat = fun(Ctrl) -> mycelium_dist_controller:getstat(Ctrl) end,
        request_type = normal,
    ...
    try
        case H of
            $N ->
                ...
                mycelium_dist_controller:set_node(Ctrl, Node);
            $n ->
...
get_stored_node(Ctrl) ->
    case mycelium_dist_controller:get_node(Ctrl) of
        {ok, Node} -> Node;
        undefined -> undefined
    end.
...
create_hs_data_setup(Kernel, DistCtrl, Node, MyNode, Type, Timer) ->
    ...
    #hs_data{
        ...
        other_flags = 0,
        f_send = fun(Ctrl, Data) -> mycelium_dist_controller:send(Ctrl, Data) end,
        f_recv = fun(Ctrl, Len, Timeout) -> mycelium_dist_controller:recv(Ctrl, Len, Timeout) end,
        f_setopts_pre_nodeup = fun(Ctrl) ->
            %% Just log and return ok - inet_tcp_dist doesn't do anything special here
            logger:info(
        ...
        f_address = fun(Ctrl, N) ->
            mycelium_dist_controller:get_address(Ctrl, N)
        end,
        mf_tick = fun(Ctrl) -> mycelium_dist_controller:tick(Ctrl) end,
        mf_getstat = fun(Ctrl) -> mycelium_dist_controller:getstat(Ctrl) end,
        request_type = Type,
...
open_stream(Node, Options) ->
    case get_controller(Node) of
        {ok, Ctrl} ->
            case mycelium_dist_controller:open_user_stream(Ctrl, self(), Options) of
                {ok, StreamId} ->
                    {ok, {mycelium_dist_stream, Node, StreamId}};
                {error, Reason} ->
                    {error, Reason}
            end;
        {error, Reason} ->
...
send({mycelium_dist_stream, Node, StreamId}, Data, Fin) ->
    case get_controller(Node) of
        {ok, Ctrl} ->
            mycelium_dist_controller:send_user_data(Ctrl, StreamId, Data, Fin);
        {error, Reason} ->
            {error, Reason}
    end.
...
%% receives {mycelium_dist_stream, StreamRef, closed}.
-spec close_stream(StreamRef :: stream_ref()) -> ok | {error, term()}.
close_stream({mycelium_dist_stream, Node, StreamId}) ->
    case get_controller(Node) of
        {ok, Ctrl} ->
            mycelium_dist_controller:close_user_stream(Ctrl, StreamId);
        {error, Reason} ->
            {error, Reason}
    end.
...
-spec reset_stream(StreamRef :: stream_ref(), ErrorCode :: non_neg_integer()) ->
    ok | {error, term()}.
reset_stream({mycelium_dist_stream, Node, StreamId}, ErrorCode) ->
    case get_controller(Node) of
        {ok, Ctrl} ->
            mycelium_dist_controller:reset_user_stream(Ctrl, StreamId, ErrorCode);
        {error, Reason} ->
            {error, Reason}
    end.
...
%%   {mycelium_dist_stream, StreamRef, {data, Data, Fin}}
%%
...
accept_streams(Node) ->
    case get_controller(Node) of
        {ok, Ctrl} ->
            mycelium_dist_controller:accept_user_streams(Ctrl, self());
        {error, Reason} ->
            {error, Reason}
    end.
...
stop_accepting(Node) ->
    case get_controller(Node) of
        {ok, Ctrl} ->
            mycelium_dist_controller:stop_accepting_streams(Ctrl);
        {error, Reason} ->
            {error, Reason}
    end.
...
controlling_process({mycelium_dist_stream, Node, StreamId}, NewOwner) ->
    case get_controller(Node) of
        {ok, Ctrl} ->
            mycelium_dist_controller:controlling_process(Ctrl, StreamId, NewOwner);
        {error, Reason} ->
            {error, Reason}
    end.
        ...
        lists:flatmap(
            fun
                ({_Node, Ctrl}) when is_pid(Ctrl) ->
                    mycelium_dist_controller:list_user_streams(Ctrl);
                ({_Node, _Port}) ->
                    []
            end,
            DistCtrls
