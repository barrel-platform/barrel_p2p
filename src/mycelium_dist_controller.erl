...
-module(mycelium_dist_controller).
...
-record(state, {
    ...
    %% Backpressure tuning (from mycelium_dist_config or defaults)
    max_pull = ?MYCELIUM_DEFAULT_MAX_PULL_PER_NOTIFICATION :: pos_integer(),
    backpressure_retry = ?MYCELIUM_DEFAULT_BACKPRESSURE_RETRY_MS :: pos_integer(),
    ...
    %% User stream support
    %% Map of StreamId -> #mycelium_user_stream{} for user-accessible streams
    user_streams = #{} :: #{non_neg_integer() => #mycelium_user_stream{}},
...
%% `{mycelium_dist_stream, StreamRef, {data, Data, Fin}}' messages. No
...
init_backpressure_config(State) ->
    ...
    MaxPull = get_dist_opt(max_pull_per_notification, DistOpts, ?MYCELIUM_DEFAULT_MAX_PULL_PER_NOTIFICATION),
    RetryMs = get_dist_opt(backpressure_retry_ms, DistOpts, ?MYCELIUM_DEFAULT_BACKPRESSURE_RETRY_MS),
    State#state{
        max_pull = MaxPull,
...
terminate(_Reason, _StateName, #state{
    ...
    maps:foreach(
        fun(_StreamId, #mycelium_user_stream{monitor = MonRef}) ->
...
setup_streams(#state{role = client} = State) ->
    ...
    case open_control_stream(State) of
        {ok, StreamId, State1} ->
            ...
            case set_stream_priority(State1, StreamId, ?MYCELIUM_DIST_URGENCY_CONTROL) of
                ok ->
                    State2 = State1#state{control_stream = StreamId},
                    %% Open data streams
                    open_data_streams(State2, ?MYCELIUM_DIST_DATA_STREAMS);
                {error, Reason} ->
...
->
    ...
    _ = set_stream_priority(State, 0, ?MYCELIUM_DIST_URGENCY_CONTROL),
...
handle_common_event(
    ...
    case maps:find(StreamId, Streams) of
        {ok, #mycelium_user_stream{owner = Owner, monitor = MonRef}} ->
            %% Notify owner of reset
            StreamRef = {mycelium_dist_stream, Node, StreamId},
            Owner ! {mycelium_dist_stream, StreamRef, {reset, ErrorCode}},
...
open_data_streams(#state{conn = Conn, data_streams = Streams} = State, N) ->
    case quic:open_stream(Conn) of
        {ok, StreamId} ->
            ...
            ok = set_stream_priority(State, StreamId, ?MYCELIUM_DIST_URGENCY_DATA_NORMAL),
...
open_server_data_streams(State) ->
    {ok, NewState} = open_data_streams(State, ?MYCELIUM_DIST_DATA_STREAMS),
    NewState.
...
%% `mycelium_dist_controller_tests:deliver_yield_returns_remainder_test/0'.
...
is_user_stream(StreamId, _State) ->
    case StreamId band 1 of
        0 ->
            ...
            StreamId >= ?MYCELIUM_USER_STREAM_THRESHOLD_CLIENT;
        1 ->
            %% Server-initiated stream (odd IDs: 1, 5, 9, 13, 17...)
            StreamId >= ?MYCELIUM_USER_STREAM_THRESHOLD_SERVER
    end.
            ...
            case is_user_stream(StreamId, State) of
                true ->
                    ...
                    Priority = validate_priority(
                        proplists:get_value(priority, Opts, ?MYCELIUM_USER_STREAM_DEFAULT_PRIORITY)
                    ),
                    ...
                    UserStream = #mycelium_user_stream{
                        id = StreamId,
                        owner = Owner,
                        monitor = MonRef,
                        priority = Priority
...
validate_priority(P) when P < ?MYCELIUM_USER_STREAM_MIN_PRIORITY -> ?MYCELIUM_USER_STREAM_MIN_PRIORITY;
...
handle_send_user_data(
    ...
    case maps:find(StreamId, Streams) of
        {ok, #mycelium_user_stream{owner = Owner} = US} ->
            ...
            case CallerPid =:= Owner of
                true ->
                    ...
                    case quic:send_data(Conn, StreamId, DataBin, Fin) of
                        ok ->
                            NewUS = US#mycelium_user_stream{send_fin = Fin},
...
handle_close_user_stream(
    ...
    case maps:find(StreamId, Streams) of
        {ok, #mycelium_user_stream{owner = Owner, monitor = MonRef, recv_fin = RecvFin} = US} ->
            %% Verify caller is the owner
            {CallerPid, _} = From,
            case CallerPid =:= Owner of
                true ->
                    ...
                    case RecvFin of
                        true ->
                            ...
                            erlang:demonitor(MonRef, [flush]),
                            StreamRef = {mycelium_dist_stream, Node, StreamId},
                            Owner ! {mycelium_dist_stream, StreamRef, closed},
                            NewStreams = maps:remove(StreamId, Streams),
                            {keep_state, State#state{user_streams = NewStreams}, [
                                {reply, From, ok}
                            ]};
                        false ->
                            ...
                            NewUS = US#mycelium_user_stream{send_fin = true},
                            NewStreams = maps:put(StreamId, NewUS, Streams),
                            {keep_state, State#state{user_streams = NewStreams}, [
                                {reply, From, ok}
                            ]}
...
handle_controlling_process(From, StreamId, NewOwner, #state{user_streams = Streams} = State) ->
    case maps:find(StreamId, Streams) of
        {ok, #mycelium_user_stream{owner = Owner, monitor = MonRef} = US} ->
            ...
            case CallerPid =:= Owner of
                true ->
                    ...
                    NewUS = US#mycelium_user_stream{owner = NewOwner, monitor = NewMonRef},
                    NewStreams = maps:put(StreamId, NewUS, Streams),
                    {keep_state, State#state{user_streams = NewStreams}, [
                        {reply, From, ok}
                    ]};
...
handle_list_user_streams(From, #state{node = Node, user_streams = Streams} = State) ->
    StreamList = maps:fold(
        fun(
            ...
            [
                #{
                    ref => {mycelium_dist_stream, Node, StreamId},
                    node => Node,
                    stream_id => StreamId,
                    owner => Owner,
                    priority => Priority,
    ...
    case lists:keyfind(Pid, 1, Pool) of
        {Pid, MonRef} ->
            ...
            {StreamsToReset, RemainingStreams} = maps:fold(
                fun(StreamId, #mycelium_user_stream{owner = O, monitor = M} = US, {ToReset, Keep}) ->
                    case O =:= Pid of
                        true ->
...
find_stream_by_monitor(MonRef, Streams) ->
    maps:fold(
        fun(StreamId, #mycelium_user_stream{monitor = MRef} = US, Acc) ->
            case MRef =:= MonRef of
                true -> {ok, StreamId, US};
                false -> Acc
            end
...
handle_user_stream_data(
    ...
    StreamRef = {mycelium_dist_stream, Node, StreamId},
    case maps:find(StreamId, Streams) of
        {ok, #mycelium_user_stream{owner = Owner, monitor = MonRef, send_fin = SendFin} = US} ->
            %% Known stream - forward data to owner
            Owner ! {mycelium_dist_stream, StreamRef, {data, Data, Fin}},
                ...
                true ->
                    ...
                    case SendFin of
                        ...
                        false ->
                            ...
                            NewUS = US#mycelium_user_stream{recv_fin = true},
                            NewStreams = maps:put(StreamId, NewUS, Streams),
                            {keep_state, State#state{
                                user_streams = NewStreams,
                                recv_cnt = RecvCnt + 1,
        ...
        error ->
            ...
            case select_acceptor(Pool, Idx) of
                {ok, AcceptorPid, NewIdx} ->
                    ...
                    AcceptorPid ! {mycelium_dist_stream, StreamRef, {data, Data, Fin}},
                    {keep_state, State#state{
                        user_streams = NewStreams,
                        acceptor_idx = NewIdx,
                        recv_cnt = RecvCnt + 1,
                ...
                {error, no_acceptor} ->
                    ...
                    _ = quic:reset_stream(Conn, StreamId, ?MYCELIUM_STREAM_REFUSED),
                    {keep_state, State#state{
                        recv_cnt = RecvCnt + 1,
                        recv_oct = RecvOct + byte_size(Data)
                    }}
