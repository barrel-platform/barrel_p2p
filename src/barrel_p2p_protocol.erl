...
-define(PROTOCOL_TAG, '$barrel_p2p_hyparview').
...
send(Node, Msg) ->
    %% Send via Erlang distribution to barrel_p2p_bridge on remote node
    erlang:send({barrel_p2p_bridge, Node}, {?PROTOCOL_TAG, node(), Msg}, [noconnect]),
    ok.

-spec handle_message({atom(), node(), term()}, pid()) -> ok.
handle_message({?PROTOCOL_TAG, From, Msg}, _Handler) ->
    barrel_p2p_hyparview:handle_msg(Msg, From),
    ok;
