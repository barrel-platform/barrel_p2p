#!/usr/bin/env bash
#
# mycelium_gen_cert.sh — generate a self-signed QUIC TLS cert for a
# mycelium node.
#
# The kernel boots `-proto_dist quic` distribution before mycelium's
...
#
# Usage:
#   mycelium_gen_cert.sh [options]
#
# Options:
#   -d, --out-dir DIR   output directory (default ./data/quic)
#   -c, --cn NAME       certificate Common Name (default mycelium)
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        -h|--help)     usage; exit 0 ;;
        *)             echo "mycelium_gen_cert: unknown arg: $1" >&2; exit 2 ;;
    esac
done

if ! command -v openssl >/dev/null 2>&1; then
    echo "mycelium_gen_cert: openssl not found in PATH" >&2
    exit 3
fi
...
if [[ -f "$CERT" && -f "$KEY" && $FORCE -eq 0 ]]; then
    echo "mycelium_gen_cert: $CERT and $KEY already exist (use --force to overwrite)"
    exit 0
fi
...
then
    echo "mycelium_gen_cert: openssl failed" >&2
    exit 3
fi

chmod 600 "$KEY"
echo "mycelium_gen_cert: wrote $CERT and $KEY (CN=$CN, valid $DAYS days)"
