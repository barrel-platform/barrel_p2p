...
between two barrel_p2p nodes. It replaces the default TCP carrier
...
The barrel_p2p proto_dist module sits on top of upstream
...
- **One connection per peer.** A single QUIC connection
  ...
  application-level streams (`barrel_p2p_streams`), and the
...
barrel_p2p expects you to trust. Peer identity is established at
the [Ed25519 layer](authentication.md), not at the TLS layer.

## Boot

A barrel_p2p node boots with three flags:
...
The first selects `barrel_p2p_dist` as Erlang's distribution
module. The second tells `net_kernel` to use barrel_p2p's
discovery shim instead of the stock EPMD daemon. The third
disables the daemon entirely; barrel_p2p does not need it.
...
## What `barrel_p2p_dist:listen/1` does

When `net_kernel` starts the listener, `barrel_p2p_dist:listen/1`
...
1. **Ensures TLS material.** If `data/quic/node.crt` and
   ...
   `barrel_p2p_quic_cert` generates a self-signed pair.
2. **Loads the `quic` and `barrel_p2p` apps.** Without this, the
   user's `sys.config` entries under `{quic, [{dist, _}]}` and
   `{barrel_p2p, _}` are invisible to `application:get_env/3`.
...
4. **Validates the projected config.** If
   `barrel_p2p.auth_enabled = true` but the projected
   `auth_callback` is `undefined` (because the user explicitly
   nulled it), boot fails with `{barrel_p2p_dist,
...
For an outgoing connection, `barrel_p2p_dist:setup/5` is the
...
1. `net_kernel:connect_node(TargetNode)` is invoked
   implicitly.
2. `barrel_p2p_dist:setup/5` opens a QUIC connection, runs the
...
`barrel_p2p_bridge` is the small gen_server that translates
...
`-proto_dist barrel_p2p`. The relevant public functions:

```erlang
%% Inspect the configured local listen port.
barrel_p2p_dist:listen_port() -> {ok, inet:port_number()} | undefined.

%% Validate a config snapshot without booting (used in tests).
barrel_p2p_dist:validate_auth_config(QuicDistOpts) -> ok.

%% Project the defaults (used in tests).
barrel_p2p_dist:project_defaults() -> ok.
```

Relevant sys.config keys (under `{barrel_p2p, [...]}`):
...
| `auth_callback` | Set by barrel_p2p; see [authentication](authentication.md). |
| `discovery_module` | Set by barrel_p2p to `barrel_p2p_discovery`. |
