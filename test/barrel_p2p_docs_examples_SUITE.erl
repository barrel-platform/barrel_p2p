...
-module(barrel_p2p_docs_examples_SUITE).
...
init_per_testcase(_Case, Config) ->
    ...
    application:set_env(barrel_p2p, barrel_p2p_map_data_dir, filename:join(Base, "maps")),
    application:set_env(barrel_p2p, reminder_data_dir, filename:join(Base, "reminders")),
...
replicated_map_flags(_Config) ->
    ...
    ?assertEqual({ok, true}, barrel_p2p:map_get(flags, dark_mode)),
    ?assertEqual(not_found, barrel_p2p:map_get(flags, missing)),
    ?assertEqual([dark_mode], barrel_p2p:map_keys(flags)),
    ?assertEqual([{dark_mode, true}], barrel_p2p:map_to_list(flags)),
    %% Validator example: a non-boolean is rejected.
    {ok, _} = barrel_p2p:new_map(vflags, #{validator => fun erlang:is_boolean/1}),
    ?assertEqual({error, invalid_value}, barrel_p2p:map_put(vflags, k, "yes")),
    %% Removal.
    ok = barrel_p2p:map_remove(flags, dark_mode),
    ?assertEqual(not_found, barrel_p2p:map_get(flags, dark_mode)).
...
durable_reminder(_Config) ->
    ...
    ok = barrel_p2p:remind_after(Key, 150, Payload),
    receive
        {barrel_p2p_reminder, K, P, Fence} ->
            ?assertEqual(Key, K),
    ...
    Key2 = {retry, 1},
    ok = barrel_p2p:remind_after(Key2, 60000, #{}),
    ok = barrel_p2p:cancel_reminder(Key2),
    receive
        {barrel_p2p_reminder, Key2, _, _} -> ct:fail(cancelled_reminder_fired)
    after 300 ->
        ok
    end.

%%====================================================================
%% docs/concepts/leader-election.md (public barrel_p2p: facade)
%%====================================================================

leader_singleton(_Config) ->
    %% Sole candidate on a single node wins immediately.
    {ok, {leader, Fence}} = barrel_p2p:lead(report_roller),
    ?assert(is_integer(Fence)),
    ?assertEqual(true, barrel_p2p:is_leader(report_roller)),
    ?assertEqual({ok, Fence}, barrel_p2p:fence(report_roller)),
    ?assertEqual({ok, node(), self()}, barrel_p2p:leader(report_roller)),
    %% Stepping down clears leadership (no `revoked' is sent to the caller).
    ok = barrel_p2p:resign(report_roller),
    ?assertEqual(false, barrel_p2p:is_leader(report_roller)),
    ?assertEqual({error, not_leader}, barrel_p2p:fence(report_roller)),
    ?assertEqual({error, no_leader}, barrel_p2p:leader(report_roller)).
...
service_registry(_Config) ->
    ok = barrel_p2p:register_service(my_worker, #{role => primary}),
    %% Local pid lookups.
    ?assertEqual({ok, self()}, barrel_p2p:lookup_local(my_worker)),
    ?assertEqual({ok, self()}, barrel_p2p:whereis_service(my_worker)),
    %% lookup/1 returns #service_entry{} records (name, pid, node, meta).
    {ok, [Entry]} = barrel_p2p:lookup(my_worker),
    ?assertMatch(
        #service_entry{
            name = my_worker,
            pid = _,
