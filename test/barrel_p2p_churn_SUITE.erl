...
test_backoff_cap(_Config) ->
    ...
    NewPassive = barrel_p2p_hyparview:record_peer_failure(Node, State),
...
test_failure_increments_count(_Config) ->
    ...
    %% First failure
    Passive1 = barrel_p2p_hyparview:record_peer_failure(Node, State),
    ?assertEqual(1, (maps:get(Node, Passive1))#peer.fail_count),

    %% Second failure
    State1 = State#view_state{passive_view = Passive1},
    Passive2 = barrel_p2p_hyparview:record_peer_failure(Node, State1),
...
test_failure_sets_backoff(_Config) ->
    ...
    %% Record failure
    Passive = barrel_p2p_hyparview:record_peer_failure(Node, State),
...
test_failure_unknown_peer(_Config) ->
    ...
    Passive = barrel_p2p_hyparview:record_peer_failure(unknown_node, State),
    ?assertEqual(#{}, Passive),
    ok.
...
test_cleanup_removes_old_entries(_Config) ->
    ...
    CleanedState = barrel_p2p_hyparview:do_cleanup_passive_view(State),
...
test_churn_join_increments(_Config) ->
    ...
    State1 = barrel_p2p_hyparview:record_churn_event(join, State),
    ?assertEqual(1, State1#view_state.recent_joins),

    State2 = barrel_p2p_hyparview:record_churn_event(join, State1),
...
test_churn_leave_increments(_Config) ->
    ...
    State1 = barrel_p2p_hyparview:record_churn_event(leave, State),
    ?assertEqual(1, State1#view_state.recent_leaves),

    State2 = barrel_p2p_hyparview:record_churn_event(leave, State1),
...
test_churn_window_reset(_Config) ->
    ...
    State1 = barrel_p2p_hyparview:maybe_reset_churn_window(StartTime + WindowMs, State),
    ...
    State2 = barrel_p2p_hyparview:maybe_reset_churn_window(StartTime + WindowMs + 1, State),
...
test_get_churn_stats(_Config) ->
    ...
    try
        ...
        {Joins, Leaves} = barrel_p2p_hyparview:get_churn_stats(),
...
test_eligible_skips_backed_off(_Config) ->
    ...
    case barrel_p2p_hyparview:find_eligible_passive_peer(State, Now) of
        {ok, Node, _Peer} ->
...
test_eligible_none_available(_Config) ->
    ...
    Result = barrel_p2p_hyparview:find_eligible_passive_peer(State, Now),
...
test_shuffle_high_churn_period(_Config) ->
    ...
    lists:foreach(
        fun(N) ->
            ...
            gen_server:cast(barrel_p2p_hyparview, {protocol_msg, {join, Peer}, NodeName})
        end,
        lists:seq(1, 15)
    ),
    ...
    %% Get churn stats
    {Joins, _Leaves} = barrel_p2p_hyparview:get_churn_stats(),
    ?assert(Joins >= 10),

    %% Verify shuffle period via sys:get_state
    {state, _, _, _, CurrentPeriod} = sys:get_state(barrel_p2p_hyparview_shuffle),
...
test_shuffle_period_bounds(_Config) ->
    ...
    State = sys:get_state(barrel_p2p_hyparview_shuffle),
...
test_churn_window_boundary(_Config) ->
    ...
    State1 = barrel_p2p_hyparview:maybe_reset_churn_window(ExactBoundary, State),
    ?assertEqual(5, State1#view_state.recent_joins),

    %% One millisecond past boundary - should reset
    PastBoundary = StartTime + WindowMs + 1,
    State2 = barrel_p2p_hyparview:maybe_reset_churn_window(PastBoundary, State),
    ?assertEqual(0, State2#view_state.recent_joins),
    ok.
...
test_rapid_failures(_Config) ->
    ...
    FinalPassive = lists:foldl(
        fun(_, Passive) ->
            ...
            barrel_p2p_hyparview:record_peer_failure(Node, CurrentState)
        end,
...
test_cleanup_during_promotion(_Config) ->
    ...
    State1 = barrel_p2p_hyparview:do_cleanup_passive_view(State0),
    Result = barrel_p2p_hyparview:find_eligible_passive_peer(State1, Now),
...
test_concurrent_churn_events(_Config) ->
    ...
    State1 = barrel_p2p_hyparview:record_churn_event(join, State),
    State2 = barrel_p2p_hyparview:record_churn_event(join, State1),
    State3 = barrel_p2p_hyparview:record_churn_event(leave, State2),
    State4 = barrel_p2p_hyparview:record_churn_event(leave, State3),
    ...
    Prop = ?FORALL(
        ...
        begin
            ...
            FinalState = lists:foldl(
                fun(Event, S) ->
                    barrel_p2p_hyparview:record_churn_event(Event, S)
                end,
                State0,
                Events
            ),
