...
%%%
%%% EUnit tests for barrel_p2p_streams.
%%%
%%% Pure in-VM tests; quic_dist is mecked.

-module(barrel_p2p_streams_tests).
...
setup() ->
    ...
    {ok, _Pid} = barrel_p2p_streams:start_link(),
    ok.
...
register_acceptor_test_() ->
    with_streams(fun() ->
        ok = barrel_p2p_streams:register_acceptor(<<"my:tag">>, self()),
        [{<<"my:tag">>, Self}] = barrel_p2p_streams:list_acceptors(),
...
register_acceptor_conflict_test_() ->
    with_streams(fun() ->
        ...
        ok = barrel_p2p_streams:register_acceptor(<<"a">>, self()),
        ?assertEqual(
            {error, conflict},
            barrel_p2p_streams:register_acceptor(<<"a">>, Other)
        ),
        exit(Other, kill)
    end).
...
register_acceptor_idempotent_test_() ->
    with_streams(fun() ->
        ok = barrel_p2p_streams:register_acceptor(<<"a">>, self()),
        ok = barrel_p2p_streams:register_acceptor(<<"a">>, self()),
        [_] = barrel_p2p_streams:list_acceptors()
    end).

unregister_acceptor_test_() ->
    with_streams(fun() ->
        ok = barrel_p2p_streams:register_acceptor(<<"a">>, self()),
        ok = barrel_p2p_streams:unregister_acceptor(<<"a">>),
...
handler_down_drops_registration_test_() ->
    with_streams(fun() ->
        ...
        ok = barrel_p2p_streams:register_acceptor(<<"x">>, Other),
        %% Give the gen_server a chance to process the DOWN.
        timer:sleep(50),
        ?assertEqual([], barrel_p2p_streams:list_acceptors())
    end).
...
open_writes_tag_preamble_test_() ->
    with_streams(fun() ->
        ...
        {ok, SR} = barrel_p2p_streams:open(<<"foo">>, 'n@h'),
...
dispatch_known_tag_handoff_test_() ->
    with_streams(fun() ->
        ...
        Preamble = <<6, "my:tag">>,
        Payload = <<"first chunk">>,
        Demuxer = whereis(barrel_p2p_streams),
...
fin_only_after_tag_preserves_close_test_() ->
    with_streams(fun() ->
        ok = barrel_p2p_streams:register_acceptor(<<"t">>, self()),
