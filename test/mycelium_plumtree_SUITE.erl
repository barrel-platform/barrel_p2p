-module(mycelium_plumtree_SUITE).
...
test_subscribe_receive_broadcast(_Config) ->
    ...
    %% Broadcast a message
    ok = mycelium_plumtree:broadcast(test_tag, <<"hello world">>),
    ...
    ok = mycelium_plumtree:unsubscribe(self()),
    ok.
...
test_unsubscribe(_Config) ->
    ...
    %% Broadcast a message
    ok = mycelium_plumtree:broadcast(test_tag, <<"test">>),
...
test_message_deduplication(_Config) ->
    ...
    ok = mycelium_plumtree:broadcast(test_tag, <<"msg1">>, MsgId),
    ok = mycelium_plumtree:broadcast(test_tag, <<"msg1">>, MsgId),
...
test_stats(_Config) ->
    %% Get initial stats
    Stats = mycelium_plumtree:get_stats(),
...
test_broadcast_with_tag(_Config) ->
    ...
    ok = mycelium_plumtree:broadcast(tag1, <<"payload1">>),
    ok = mycelium_plumtree:broadcast(tag2, #{key => value}),
...
test_multiple_subscribers(_Config) ->
    ...
    ok = mycelium_plumtree:subscribe(Sub1),
    ok = mycelium_plumtree:subscribe(Sub2),

    %% Broadcast a message
    ok = mycelium_plumtree:broadcast(multi_test, <<"shared message">>),
