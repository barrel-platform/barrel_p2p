...
%%%
-module(barrel_p2p_docker_auth_SUITE).
...
init_per_suite(Config) ->
    case os:getenv("TEST_NODES") of
        ...
        NodesStr ->
            ...
            ok = wait_for_barrel_p2p_auth(Nodes, 30000),
            ct:pal("Barrel P2P with auth running on all nodes"),
...
test_auth_enabled_on_all_nodes(Config) ->
    ...
    lists:foreach(
        fun(Node) ->
            AuthEnabled = rpc:call(Node, application, get_env, [barrel_p2p, auth_enabled, false]),
...
test_keys_generated_on_all_nodes(Config) ->
    ...
    lists:foreach(
        fun(Node) ->
            PubKeyResult = rpc:call(Node, barrel_p2p_dist_auth, get_public_key, []),
...
test_public_key_accessible(Config) ->
    ...
    %% Get public key
    {ok, PubKey} = rpc:call(Node1, barrel_p2p_dist_auth, get_public_key, []),
    ...
    {ok, PubKey2} = rpc:call(Node1, barrel_p2p_dist_auth, get_public_key, []),
...
test_tofu_keys_persisted(Config) ->
    ...
    KeyDir = rpc:call(Node1, application, get_env, [barrel_p2p, auth_key_dir, "data/keys"]),
...
wait_active_view_nonempty(Nodes, Deadline) ->
    case
        lists:all(
            fun(Node) ->
                Active = rpc:call(Node, barrel_p2p, active_view, []),
...
test_service_registration_authenticated(Config) ->
    ...
    RegResult = rpc:call(Node1, barrel_p2p, register_service, [ServiceName, #{}]),
    ...
    Services = rpc:call(Node2, barrel_p2p, list_services, []),
    ct:pal("Services on Node2: ~p", [Services]),
    ?assert(is_list(Services)),

    %% Cleanup
    rpc:call(Node1, barrel_p2p, unregister_service, [ServiceName]),
...
test_node_rejoin_authenticated(Config) ->
    ...
    %% Leave cluster
    ok = rpc:call(Node3, barrel_p2p, leave, []),
    timer:sleep(1000),

    %% Rejoin - should re-authenticate
    ok = rpc:call(Node3, barrel_p2p, join, [Node1]),
    timer:sleep(3000),
...
test_whitelist_config(Config) ->
    ...
    Whitelist = rpc:call(Node1, application, get_env, [barrel_p2p, cookie_only_nodes, []]),
...
test_whitelist_pattern_matching(Config) ->
    ...
    %% Test exact match
    ExactMatch = rpc:call(Node1, barrel_p2p_dist_auth, is_cookie_only_allowed, ['cnode@localhost']),
    ct:pal("Exact match 'cnode@localhost': ~p", [ExactMatch]),
    ?assert(ExactMatch),

    %% Test wildcard host match
    WildcardHost = rpc:call(Node1, barrel_p2p_dist_auth, is_cookie_only_allowed, ['monitor@anyhost']),
    ct:pal("Wildcard host 'monitor@anyhost': ~p", [WildcardHost]),
    ?assert(WildcardHost),

    %% Test wildcard name match
    WildcardName = rpc:call(Node1, barrel_p2p_dist_auth, is_cookie_only_allowed, [
    ...
    %% Test no match
    NoMatch = rpc:call(Node1, barrel_p2p_dist_auth, is_cookie_only_allowed, ['random@random']),
    ct:pal("No match 'random@random': ~p", [NoMatch]),
    ?assertNot(NoMatch),

    %% Clean up - reset to empty whitelist
    ok = rpc:call(Node1, application, set_env, [barrel_p2p, cookie_only_nodes, []]),
    ok.
...
test_automatic_cookie(Config) ->
    ...
    Cookie1 = rpc:call(Node1, application, get_env, [barrel_p2p, dist_cookie, undefined]),
    Cookie2 = rpc:call(Node2, application, get_env, [barrel_p2p, dist_cookie, undefined]),
...
wait_for_barrel_p2p_auth(Nodes, Timeout) ->
    Deadline = erlang:monotonic_time(millisecond) + Timeout,
    wait_for_barrel_p2p_auth_loop(Nodes, Deadline).

wait_for_barrel_p2p_auth_loop([], _Deadline) ->
    ok;
wait_for_barrel_p2p_auth_loop([Node | Rest], Deadline) ->
    Now = erlang:monotonic_time(millisecond),
    case Now >= Deadline of
        true ->
            {error, {barrel_p2p_not_running, Node}};
        false ->
            Apps = rpc:call(Node, application, which_applications, [], 2000),
            case is_list(Apps) andalso lists:keyfind(barrel_p2p, 1, Apps) of
                false ->
                    timer:sleep(500),
                    wait_for_barrel_p2p_auth_loop([Node | Rest], Deadline);
                _ ->
                    ...
                    AuthEnabled = rpc:call(
                        ...
                        application,
                        get_env,
                        [barrel_p2p, auth_enabled, false],
                        2000
                    ),
                    case AuthEnabled of
                        true ->
                            wait_for_barrel_p2p_auth_loop(Rest, Deadline);
                        _ ->
                            timer:sleep(500),
                            wait_for_barrel_p2p_auth_loop([Node | Rest], Deadline)
                    end
