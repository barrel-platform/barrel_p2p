...
%%%
%%% Soak / chaos suite for barrel_p2p.
%%%
%%% Gated by `BARREL_P2P_CT_SOAK=1' so casual CI runs skip it.
%%%
%%% Active case:
%%%
...
all() ->
    case os:getenv("BARREL_P2P_CT_SOAK") of
        false ->
            {skip, "set BARREL_P2P_CT_SOAK=1 to run soak suite"};
        "" ->
            {skip, "BARREL_P2P_CT_SOAK is empty"};
        _ ->
...
broadcast_burst(Config) ->
    ...
    lists:foreach(
        fun
            ({PX, _NodeX, _, _}) when PX =/= PA1 ->
                ok = peer:call(PX, barrel_p2p, join, [NodeA]);
            (_) ->
                ok
        end,
        Peers
            ...
            lists:all(
                fun
                    ({PX, _, _, _}) when PX =/= PA1 ->
                        AV = peer:call(PX, barrel_p2p, active_view, []),
    ...
    wait_until(
        fun() ->
            SeedAV = peer:call(PA1, barrel_p2p, active_view, []),
            lists:all(
    ...
    ok = peer:call(PA1, barrel_p2p_plumtree, broadcast, [soak, Marker]),
    try
        wait_until(
            fun() ->
                lists:all(
        ...
        _:_ ->
            ...
            Stats = [peer:call(P, barrel_p2p_plumtree, get_stats, []) || P <- Survivors],
...
partition_and_heal(Config) ->
    ...
    ok = peer:call(Pb, barrel_p2p, join, [NodeA]),
    ok = peer:call(Pd, barrel_p2p, join, [NodeC]),
    wait_until(fun() -> in_view(Pa, [NodeB]) andalso in_view(Pc, [NodeD]) end, 15000),
    ok = peer:call(Pc, barrel_p2p, join, [NodeA]),
    ...
    ok = peer:call(Pa, barrel_p2p_hyparview, block_peers, [[NodeC, NodeD]]),
    ok = peer:call(Pb, barrel_p2p_hyparview, block_peers, [[NodeC, NodeD]]),
    ok = peer:call(Pc, barrel_p2p_hyparview, block_peers, [[NodeA, NodeB]]),
    ok = peer:call(Pd, barrel_p2p_hyparview, block_peers, [[NodeA, NodeB]]),
    ...
    [ok = peer:call(P, barrel_p2p_hyparview, unblock_peers, []) || P <- [Pa, Pb, Pc, Pd]],
    ok = peer:call(Pc, barrel_p2p, join, [NodeA]),
    ok = peer:call(Pb, barrel_p2p, join, [NodeC]),
    ok = peer:call(Pd, barrel_p2p, join, [NodeA]),
    wait_until(
        fun() ->
            lists:all(
...
visible(P, Svc) ->
    case peer:call(P, barrel_p2p, lookup, [Svc]) of
        {ok, [_ | _]} -> true;
        _ -> false
    end.
...
cross_active_view_bang(Config) ->
    ...
    lists:foreach(
        fun
            ({P, _, _, _}) when P =/= Pa1 ->
                ok = peer:call(P, barrel_p2p, join, [NodeA]);
            (_) ->
                ok
        end,
        Peers
...
%% advertise it through the barrel_p2p service registry.
...
start_collector() ->
    case whereis(soak_collector) of
        undefined ->
            ...
            barrel_p2p_plumtree:subscribe(Pid);
        _ ->
            ok
    end,
    ok.
...
%%====================================================================
%% Peer setup (shared shape with barrel_p2p_proto_dist_SUITE)
%%====================================================================
...
start_peer(Suffix, Config, Opts) ->
    ...
    AuthArgs =
        case maps:get(auth_enabled, Opts, undefined) of
            false -> ["-barrel_p2p", "auth_enabled", "false"];
            true -> ["-barrel_p2p", "auth_enabled", "true"];
            _ -> []
        end,
    BaseArgs =
        [
    ...
    {ok, _} = peer:call(Pid, application, ensure_all_started, [barrel_p2p]),
    put(?MODULE, [
        Pid
        | case get(?MODULE) of
            undefined -> [];
