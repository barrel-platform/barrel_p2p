...
quic_call_path() ->
    case code:priv_dir(quic) of
        {error, bad_name} -> error({quic_priv_dir_not_found});
        Priv ->
...
start_slave(Short, ParentPriv, Spec, _ReadyTimeoutMs) ->
    ...
    EbinPath = lists:flatten(
        [["-pa ", P, " "] || P <- code:get_path(),
                              P =/= ".", P =/= ""]),

    Args = string:tokens(EbinPath, " ") ++ [
...
qcall(#{name := Name, cookie := Cookie, probe_config := Cfg}, M, F, A, Timeout) ->
    ...
    case parse_term(Out) of
        {error, _} = Err ->
            ct:pal("qcall(~p, ~p, ~p, ~p) failed:~n~ts",
                   [Name, M, F, A, Out]),
            Err;
        Term -> Term
    end.
