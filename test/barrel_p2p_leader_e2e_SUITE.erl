...
%%% under `-proto_dist barrel_p2p', forms a fully-connected cluster, and
%%% exercises election, peer_down re-election, and fencing across a live
%%% node death. The peer scaffolding mirrors barrel_p2p_proto_dist_SUITE.
%%%
%%% Candidates must be long-lived (barrel_p2p monitors the campaigning
...
-module(barrel_p2p_leader_e2e_SUITE).
...
leader_failover_increments_fence(Config) ->
    ...
    wait_until(
        fun() ->
            lists:all(
                fun({P, _N}) ->
                    case peer:call(P, barrel_p2p_leader, high_water, [job]) of
    ...
    wait_until(
        fun() ->
            lists:all(
                fun({P, _N}) ->
                    peer:call(P, barrel_p2p, leader, [job]) =:= {ok, NewExpected, NewPid}
                end,
                Survivors
            )
        end,
...
start_candidate(Name, Priority) ->
    ...
    spawn(fun() ->
        {Role, Fence} =
            case barrel_p2p:lead(Name, #{priority => Priority}) of
...
candidate_loop(Name, Role, Fence) ->
    receive
        {barrel_p2p_leader, Name, {elected, F}} ->
...
agree_leader(Peers, Name, ExpectedNode) ->
    ...
    is_pid(EPid) andalso
        lists:all(
            fun({P, _N}) ->
                peer:call(P, barrel_p2p, leader, [Name]) =:= {ok, ExpectedNode, EPid}
            end,
            Peers
        ).

%%====================================================================
%% Peer setup (mirrors barrel_p2p_proto_dist_SUITE:start_peer/3)
%%====================================================================
