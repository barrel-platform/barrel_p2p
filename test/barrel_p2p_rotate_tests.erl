...
rotate_identity_fresh_test() ->
    with_tmp(fun(Tmp) ->
        {ok, Info} = barrel_p2p_rotate:rotate_identity(#{key_dir => Tmp}),
...
rotate_identity_backs_up_test() ->
    with_tmp(fun(Tmp) ->
        ...
        {ok, _} = barrel_p2p_rotate:rotate_identity(#{key_dir => Tmp}),
...
rotate_cert_fresh_test() ->
    with_tmp(fun(Tmp) ->
        {ok, Info} = barrel_p2p_rotate:rotate_cert(#{cert_dir => Tmp}),
        ?assertEqual(undefined, maps:get(backup_dir, Info)),
...
rotate_cert_backs_up_test() ->
    with_tmp(fun(Tmp) ->
        {ok, _} = barrel_p2p_rotate:rotate_cert(#{cert_dir => Tmp}),
