...
%%%
%%% EUnit tests for barrel_p2p_path_stats.
%%%
%%% The success path (sys:get_state on a live dist_controller +
%%% quic:get_path_stats on the conn pid) is exercised by
%%% barrel_p2p_circuit_multinode_SUITE; here we just verify the error
%%% paths through the wrapper.

-module(barrel_p2p_path_stats_tests).
...
summary_propagates_not_connected_test_() ->
    with(fun() ->
        ...
        ?assertEqual(
            {error, not_connected},
            barrel_p2p_path_stats:summary('peer@h')
        )
    end).
...
srtt_propagates_not_connected_test_() ->
    with(fun() ->
        ...
        ?assertEqual(
            {error, not_connected},
            barrel_p2p_path_stats:srtt('peer@h')
        )
    end).
...
connection_propagates_not_connected_test_() ->
    with(fun() ->
        ...
        ?assertEqual(
            {error, not_connected},
            barrel_p2p_path_stats:connection('peer@h')
        )
    end).
...
%% sys:get_state. We stand in a real gen_server (barrel_p2p_test_server)
...
fake_controller(StateTuple) ->
    {ok, Ctrl} = barrel_p2p_test_server:start_link(),
...
summary_fast_path_returns_stats_test() ->
    ...
    try
        ...
        ?assertEqual({ok, Stats}, barrel_p2p_path_stats:summary('peer@h')),
        ?assertEqual({ok, 4321}, barrel_p2p_path_stats:srtt('peer@h')),
        ?assertEqual({ok, Conn}, barrel_p2p_path_stats:connection('peer@h'))
    after
...
summary_scan_exhausted_returns_no_conn_test() ->
    ...
    try
        ...
        ?assertEqual({error, no_conn}, barrel_p2p_path_stats:summary('peer@h'))
    after
        gen_server:stop(Ctrl),
        Conn ! stop,
        meck:unload(quic),
