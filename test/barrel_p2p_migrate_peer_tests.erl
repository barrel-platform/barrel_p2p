...
%%%
%%% EUnit tests for barrel_p2p:migrate_peer/1,2.
%%%
...
%%% record shape that `barrel_p2p_path_stats:extract_conn/1' relies on).

-module(barrel_p2p_migrate_peer_tests).
...
migrate_peer_propagates_not_connected_test_() ->
    with(fun() ->
        ...
        ?assertEqual(
            {error, not_connected},
            barrel_p2p:migrate_peer('peer@h')
        )
    end).
