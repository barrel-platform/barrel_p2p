...
%%%
%%% barrel_p2p cluster basic Common Test suite.
%%%
%%% Spawns slave nodes via `ct_slave' over plain `inet_tcp_dist' and
%%% runs the barrel_p2p application on each. Tests cluster mechanics
%%% (HyParView join, active view, service registry, message round-
%%% trip) without standing up Docker or making the CT BEAM itself a
%%% barrel_p2p_dist node. Local-CT replacement for the docker-based
%%% `barrel_p2p_integration_SUITE'.
%%%

-module(barrel_p2p_dist_basic_SUITE).
...
init_per_suite(Config) ->
    ...
    erlang:set_cookie(node(), barrel_p2p_ct_cookie),
...
%% Two peers can connect over barrel_p2p_dist via barrel_p2p:join/1.
node_connect_test(Config) ->
    Node1 = ?config(node1, Config),
    Node2 = ?config(node2, Config),

    ok = rpc:call(Node2, barrel_p2p, join, [Node1]),
    wait_until(
        fun() ->
...
active_view_test(Config) ->
    ...
    wait_until(
        fun() ->
            Active1 = rpc:call(Node1, barrel_p2p, active_view, []),
            Active2 = rpc:call(Node2, barrel_p2p, active_view, []),
...
register_service_test(Config) ->
    ...
    %% Spawn a holder process on Node1: barrel_p2p_registry monitors the
    ...
    Services = rpc:call(Node1, barrel_p2p, list_services, []),
    ?assert(is_list(Services)),
    ?assert(lists:member(my_service, Services)),

    rpc:call(Node1, barrel_p2p, unregister_service, [my_service]),
    Holder ! stop,
    ok.
...
list_services_test(Config) ->
    Node1 = ?config(node1, Config),
    Result = rpc:call(Node1, barrel_p2p, list_services, []),
...
leave_rejoin_test(Config) ->
    ...
    ok = rpc:call(Node2, barrel_p2p, leave, []),
    wait_until(
        fun() ->
            Active = rpc:call(Node2, barrel_p2p, active_view, []),
...
registry_replica_running_test(Config) ->
    lists:foreach(
        fun(Node) ->
            Pid = rpc:call(Node, erlang, whereis, [barrel_p2p_registry_replica]),
            ?assert(is_pid(Pid))
        end,
...
whereis_service_remote_test(Config) ->
    ...
    after
        rpc:call(Node2, barrel_p2p, stop_service_holder, [HolderPid])
    end,
    ok.
...
shuffle_test(Config) ->
    Node1 = ?config(node1, Config),
    ok = rpc:call(Node1, barrel_p2p_hyparview_shuffle, trigger_shuffle, []),
    timer:sleep(500),
    Active = rpc:call(Node1, barrel_p2p, active_view, []),
...
%% clobber you get with `-proto_dist inet_tcp barrel_p2p'). Each slave
%% has the barrel_p2p application running with its own QUIC listen_port,
%% which is what HyParView/registry/circuit code under test actually
%% uses; barrel_p2p_dist as a dist carrier is exercised by other suites.
start_peer_nodes() ->
    Port1 = 14433,
    Port2 = 14434,
    ...
    %% from each other before barrel_p2p:join links them.
    ErlFlags = fun() ->
    ...
    try
        ...
        lists:foreach(
            fun({Node, Port}) ->
                %% Keep the slave on the parent's cookie - barrel_p2p_app's
                %% init_dist_cookie/0 otherwise resets it to `barrel_p2p'
