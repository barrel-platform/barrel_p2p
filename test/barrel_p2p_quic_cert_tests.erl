...
%%%
%%% EUnit tests for barrel_p2p_quic_cert validity-date encoding and
%%% serial-number derivation.

-module(barrel_p2p_quic_cert_tests).
...
default_validity_uses_utc_time_today_test() ->
    Dir = tmp_dir(),
    try
        ok = barrel_p2p_quic_cert:ensure_cert(Dir),
...
serials_are_distinct_across_generations_test() ->
    ...
    Dir2 = tmp_dir(),
    try
        ok = barrel_p2p_quic_cert:ensure_cert(Dir1),
        ok = barrel_p2p_quic_cert:ensure_cert(Dir2),
