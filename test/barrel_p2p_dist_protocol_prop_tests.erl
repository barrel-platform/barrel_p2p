...
%%%
%%% Property tests for barrel_p2p_dist_protocol.
%%%
%%% Two flavours:
%%%
...
-module(barrel_p2p_dist_protocol_prop_tests).
...
prop_challenge_roundtrip() ->
    ?FORALL(
        ...
        begin
            Enc = barrel_p2p_dist_protocol:encode_challenge(Nonce, TS),
            {challenge, Nonce, TS} =:= barrel_p2p_dist_protocol:decode(Enc)
        end
    ).
...
prop_response_roundtrip() ->
    ?FORALL(
        ...
        begin
            Enc = barrel_p2p_dist_protocol:encode_response(Sig),
            {response, Sig} =:= barrel_p2p_dist_protocol:decode(Enc)
        end
    ).
...
prop_fail_roundtrip() ->
    ?FORALL(
        ...
        binary(),
        begin
            Enc = barrel_p2p_dist_protocol:encode_fail(Reason),
            {fail, Reason} =:= barrel_p2p_dist_protocol:decode(Enc)
        end
    ).
...
prop_ok_roundtrip() ->
    ?FORALL(
        ...
        integer(),
        ok =:=
            barrel_p2p_dist_protocol:decode(
                barrel_p2p_dist_protocol:encode_ok()
            )
    ).
...
prop_decode_random_never_crashes() ->
    ?FORALL(
        Bin,
        binary(),
        try barrel_p2p_dist_protocol:decode(Bin) of
            ok -> true;
            {error, _} -> true;
            {hello, _, _} -> true;
            {challenge, _, _} -> true;
