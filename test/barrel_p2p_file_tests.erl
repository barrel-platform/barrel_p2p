...
%%%
%%% EUnit tests for barrel_p2p_file:write_secure/2.

-module(barrel_p2p_file_tests).
...
write_secure_creates_file_with_0600_test() ->
    {Dir, Path} = tmp_path(),
    try
        ok = barrel_p2p_file:write_secure(Path, <<"hello">>),
...
write_secure_no_tmp_after_success_test() ->
    {Dir, Path} = tmp_path(),
    try
        ok = barrel_p2p_file:write_secure(Path, <<"abc">>),
...
write_secure_overwrites_existing_test() ->
    {Dir, Path} = tmp_path(),
    try
        ok = barrel_p2p_file:write_secure(Path, <<"first">>),
        ok = barrel_p2p_file:write_secure(Path, <<"second">>),
        {ok, Data} = file:read_file(Path),
...
write_secure_returns_error_on_bad_path_test() ->
    ...
    ?assertMatch({error, _}, barrel_p2p_file:write_secure(BadPath, <<"x">>)).
