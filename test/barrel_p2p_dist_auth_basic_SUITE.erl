...
%%%
%%% barrel_p2p auth basic Common Test suite.
%%%
%%% Local-CT counterpart of `barrel_p2p_docker_auth_SUITE'. Spawns three
%%% slave nodes via `ct_slave', each with its own `auth_key_dir', and
%%% drives `barrel_p2p_dist_keys' / `barrel_p2p_dist_auth' over RPC. We do
%%% not stand the slaves up on `-proto_dist barrel_p2p' here, so the
%%% Ed25519 handshake driven by `barrel_p2p_dist_protocol' is not
...
-module(barrel_p2p_dist_auth_basic_SUITE).
...
init_per_suite(Config) ->
    ...
    erlang:set_cookie(node(), barrel_p2p_auth_ct_cookie),
    %% Earlier suites may have started the barrel_p2p application on the
...
keys_generated_test(Config) ->
    ...
    lists:foreach(
        fun({Node, KeyDir}) ->
            ...
            {ok, _} = rpc:call(Node, barrel_p2p_dist_auth, get_public_key, []),
            {ok, _} = rpc:call(Node, barrel_p2p_dist_auth, get_private_key, [])
        end,
        Nodes
    ),
    ok.

%% Restarting barrel_p2p_dist_keys reloads the same keypair from disk.
keypair_persisted_test(Config) ->
    [{Node, _KeyDir} | _] = ?config(nodes, Config),

    {ok, PubKeyBefore} = rpc:call(Node, barrel_p2p_dist_auth, get_public_key, []),
    ok = rpc:call(Node, application, stop, [barrel_p2p]),
    {ok, _} = rpc:call(Node, application, ensure_all_started, [barrel_p2p]),
    {ok, PubKeyAfter} = rpc:call(Node, barrel_p2p_dist_auth, get_public_key, []),
...
trust_mode_test(Config) ->
    ...
    ok = rpc:call(Node, barrel_p2p_dist_keys, set_trust_mode, [strict]),
    strict = rpc:call(Node, barrel_p2p_dist_keys, get_trust_mode, []),
    ok = rpc:call(Node, barrel_p2p_dist_keys, set_trust_mode, [tofu]),
    tofu = rpc:call(Node, barrel_p2p_dist_keys, get_trust_mode, []),
    ok.
...
tofu_store_if_new_test(Config) ->
    ...
    ok = rpc:call(Node, barrel_p2p_dist_keys, delete_key, [PeerNode]),
    ok = rpc:call(Node, barrel_p2p_dist_keys, set_trust_mode, [tofu]),

    Key1 = crypto:strong_rand_bytes(32),
    Key2 = crypto:strong_rand_bytes(32),

    ok = rpc:call(Node, barrel_p2p_dist_keys, store_key_if_new, [PeerNode, Key1]),
    ?assert(rpc:call(Node, barrel_p2p_dist_keys, is_trusted, [PeerNode, Key1])),

    %% Second store with a different key must not overwrite.
    rpc:call(Node, barrel_p2p_dist_keys, store_key_if_new, [PeerNode, Key2]),
    ?assertNot(rpc:call(Node, barrel_p2p_dist_keys, is_trusted, [PeerNode, Key2])),
    ?assert(rpc:call(Node, barrel_p2p_dist_keys, is_trusted, [PeerNode, Key1])),
...
strict_mode_rejects_change_test(Config) ->
    ...
    ok = rpc:call(Node, barrel_p2p_dist_keys, store_key, [PeerNode, Key1]),
    ?assert(rpc:call(Node, barrel_p2p_dist_keys, is_trusted, [PeerNode, Key1])),
    ?assertNot(rpc:call(Node, barrel_p2p_dist_keys, is_trusted, [PeerNode, Key2])),
...
list_trusted_test(Config) ->
    ...
    ok = rpc:call(Node, barrel_p2p_dist_keys, store_key, [PeerA, KeyA]),
    ok = rpc:call(Node, barrel_p2p_dist_keys, store_key, [PeerB, KeyB]),

    Trusted = rpc:call(Node, barrel_p2p_dist_keys, list_trusted, []),
    Nodes = [element(2, T) || T <- Trusted, is_tuple(T)],
    ?assert(lists:member(PeerA, Nodes)),
    ?assert(lists:member(PeerB, Nodes)),

    ok = rpc:call(Node, barrel_p2p_dist_keys, delete_key, [PeerA]),
    ok = rpc:call(Node, barrel_p2p_dist_keys, delete_key, [PeerB]),
    ok.
...
start_peer_nodes(PrivDir) ->
    ...
    Paths = [
     ...
     || P <- code:get_path(),
        P =/= ".",
        filename:basename(P) =/= "erlang-barrel_p2p",
        P =/= ""
    ],
    CodePath = string:join(Paths, " "),

    ErlFlags =
        "-barrel_p2p auth_enabled true "
        "-barrel_p2p auth_trust_mode tofu "
        ...
        Nodes = lists:map(
            fun({Prefix, Port}) ->
                ...
                ok = rpc:call(
                    ...
                    application,
                    set_env,
                    [barrel_p2p, dist_cookie, Cookie]
                ),
                ok = rpc:call(
                    Node,
                    application,
                    set_env,
                    [barrel_p2p, listen_port, Port]
                ),
                ok = rpc:call(
                    Node,
                    application,
                ...
                ok = rpc:call(
                    ...
                    application,
                    set_env,
                    [barrel_p2p, auth_key_dir, KeyDir]
                ),
                {ok, _} = rpc:call(
                    Node,
                    application,
