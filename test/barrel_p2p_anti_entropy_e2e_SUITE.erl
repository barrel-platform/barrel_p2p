...
%%% (here a barrel_p2p_map) WITHOUT a fresh peer_up. It is an isolating A/B
...
-module(barrel_p2p_anti_entropy_e2e_SUITE).
...
start_pair(Config, AeMs) ->
    ...
    {ok, _} = peer:call(Pa, barrel_p2p, new_map, [m]),
    {ok, _} = peer:call(Pb, barrel_p2p, new_map, [m]),
    ...
    wait_until(
        fun() ->
            lists:member(NodeA, peer:call(Pb, barrel_p2p, active_view, [])) andalso
                lists:member(NodeB, peer:call(Pa, barrel_p2p, active_view, []))
        end,
        30000
    ),
    ...
    Rep = peer:call(Pa, barrel_p2p_map, replica_name, [m]),
    timer:sleep(2000),
...
inject(Pa) ->
    HLC = peer:call(Pa, barrel_p2p_hlc, now, []),
    Delta = #{k => {value, v, #{{'origin@127.0.0.1', HLC} => true}}},
    Rep = peer:call(Pa, barrel_p2p_map, replica_name, [m]),
    ok = peer:call(Pa, barrel_p2p_map, replica_merge_delta, [Rep, Delta]),
...
%%====================================================================
%% Peer setup (mirrors barrel_p2p_reminder_e2e_SUITE)
%%====================================================================
