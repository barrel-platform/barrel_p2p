...
%%% `-proto_dist barrel_p2p' and exercises the round-2 fixes against
%%% the live dist handshake and supervision tree.
%%%
%%% Reuses the `start_peer/3' helper from barrel_p2p_proto_dist_SUITE
%%% (export_all). Each case picks the scaffolding it needs.

-module(barrel_p2p_audit_e2e_SUITE).
...
tofu_repin_rejected(Config) ->
    ...
    wait_until(
        fun() ->
            case peer:call(Pa, barrel_p2p_dist_keys, lookup_pin, [NodeB]) of
                {pinned, _} -> true;
                _ -> false
            end
        end,
    ...
    {pinned, OldPub} = peer:call(Pa, barrel_p2p_dist_keys, lookup_pin, [NodeB]),
    ...
    ?assertEqual(
        {pinned, OldPub},
        peer:call(Pa, barrel_p2p_dist_keys, lookup_pin, [NodeB])
    ),
    ok.
...
pending_timeout_clears_phantom_join(Config) ->
    ...
    _ = peer:call(Pa, barrel_p2p, join, ['phantom@127.0.0.1']),
    wait_until(
        fun() ->
...
keypair_mismatch_detected_on_load(Config) ->
    ...
    ?assertMatch(
        {ok, _, _},
        peer:call(Pa, barrel_p2p_dist_auth, load_keypair, [KeyDir])
    ),
...
validate_peer_ts_in_handshake(Config) ->
    ...
    ?assertEqual(
        ok,
        peer:call(Pa, barrel_p2p_dist_auth, validate_peer_ts, [Now])
    ),
...
%%====================================================================
%% Peer scaffolding (delegates to barrel_p2p_proto_dist_SUITE)
%%====================================================================
...
restart_peer(Suffix, Config, Opts) ->
    ...
    %% under the original path. barrel_p2p_proto_dist_SUITE doesn't
    %% expose a restart helper; reusing start_peer/3 is enough.
    do_start_peer(Suffix, Config, Opts).

%% Patched copy of barrel_p2p_proto_dist_SUITE:start_peer/3 that
%% understands a few extra knobs:
%%   - cookie_only_nodes :: [atom()]    -> -barrel_p2p cookie_only_nodes
%%   - pending_timeout_ms :: pos_integer() -> -barrel_p2p pending_timeout_ms
do_start_peer(Suffix, Config, Opts) ->
    ...
    PendingArgs =
        case Opts of
            #{pending_timeout_ms := PT} ->
                ["-barrel_p2p", "pending_timeout_ms", integer_to_list(PT)];
            _ ->
                []
        end,
